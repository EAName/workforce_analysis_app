...
- Robust logging system with rotating file handlers
...
├── data/               # Data storage
├── logs/               # Application logs (git-ignored, created automatically)
...
Note: The `logs/` directory is not tracked in Git (git-ignored) as it contains environment-specific log files that are generated automatically when the application runs. Each environment (development, staging, production) maintains its own logs.
